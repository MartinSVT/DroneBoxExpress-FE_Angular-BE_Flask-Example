...
class NewsArticleModel(db.Model):
    ...
    created_date: Mapped[datetime] = mapped_column(db.DateTime, server_default=func.now())
    updated_date = db.Column(db.DateTime, onupdate=func.now())
    article_user: Mapped[int] = mapped_column(db.Integer, db.ForeignKey('users.id', ondelete="CASCADE"))
