...
class OrdersModel(db.Model):
    ...
    order_route: Mapped[int] = mapped_column(db.Integer, db.ForeignKey('Routes.id', ondelete="CASCADE"))
    order_user: Mapped[int] = mapped_column(db.Integer, db.ForeignKey('users.id', ondelete="CASCADE"))

    o_user: Mapped["UserModel"] = relationship(
        "UserModel",
        foreign_keys=[order_user],
        back_populates="user_orders"
    )

    o_route: Mapped["RoutesModel"] = relationship(
        "RoutesModel",
        foreign_keys=[order_route],
        back_populates="route_orders"
    )
