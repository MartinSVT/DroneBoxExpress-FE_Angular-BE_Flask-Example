...
class CreateUpdateOrderSchema(Schema):
    ...
    order_status = fields.String(required=False, validate=OneOf(["Completed", "Scheduled"]))

    @validates("weight")
    def validates_weight(self, weight):
        if weight < 0:
            raise ValidationError("Must be Positive Number", field_names=["weight"], )

    @validates("cost")
    def validates_cost(self, cost):
        if cost < 0:
            raise ValidationError("Must be Positive Number", field_names=["cost"], )
    ...
    def validates_order_route(self, order_route):
        if not RoutesModel.query.filter(RoutesModel.id == order_route).first():
            raise ValidationError("No Such Route Exists", field_names=["order_route"], )
    ...
    def validates_order_user(self, order_user):
        ...
        if not user.is_staff:
            ...
            if not UserModel.query.filter(UserModel.id == order_user).first():
                raise ValidationError("No Such User", field_names=["order_user"], )
